...
import { Loader2, ChevronDown, ChevronRight, FileDown, Search } from "lucide-react";
...
export function HallmarkTab({ orders: propOrders, isError }: HallmarkTabProps) {
  ...
  const karigars = useMemo(() => {
    const set = new Set(hallmarkOrders.map((o) => o.karigarName ?? "").filter(Boolean));
  ...
  const handleMarkAsReturned = () => {
    ...
    batchUpdateMutation.mutate(
      { orderIds: Array.from(selectedIds), newStatus: OrderStatus.ReturnFromHallmark },
  ...
  const handleExportJpeg = async () => {
    ...
    try {
      await exportOrdersToImage(filtered, "Hallmark Orders", "hallmark-orders.jpg");
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">No hallmark orders found.</div>
      ) : (
