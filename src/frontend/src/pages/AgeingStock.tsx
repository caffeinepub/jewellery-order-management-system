...
export default function AgeingStock() {
  ...
  const toggleGroup = (_designCode: string, orders: EnrichedOrder[]) => {
    ...
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="space-y-3">
        {groups.map((group) => {
          ...
          const allGroupSelected = group.orders.every((o) =>
            selectedIds.has(o.orderId),
          );
          const someGroupSelected = group.orders.some((o) =>
            selectedIds.has(o.orderId),
          );
          ...
          return (
            <Card
              ...
              className={`overflow-hidden bg-card ${ageBandLeftBorderClasses[groupBand]}`}
            >
              <CardHeader
                className="py-3 px-4 cursor-pointer select-none"
                onClick={() => toggleExpanded(group.designCode)}
              >
                <div className="flex items-center gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      toggleGroup(group.designCode, group.orders)
                    }
                  />
                ...
                <CardContent className="p-0">
                  <div className="divide-y">
                    {group.orders.map((order) => {
                      ...
                      return (
                        // biome-ignore lint/a11y/useKeyWithClickEvents: row selection with nested interactive elements
                        <div
                          key={order.orderId}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-muted-foreground">
                              ...
                              <span>
                                Wt:{" "}
                                {(
                                  order.weight * Number(order.quantity)
                                ).toFixed(2)}
                                g
                              </span>
                              {order.remarks && (
                          ...
                          <div className="flex-shrink-0">
                            {order.ageDays !== null ? (
                              <span
                                className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${ageBandBadgeClasses[band]}`}
                              >
                                {order.ageDays}d
                              </span>
                            ) : (
                              <span
                                className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${ageBandBadgeClasses.none}`}
                              >
                                No date
                              </span>
                            )}
